...
class Burrow {
  ...
  public List<String> getClusterBootstrapServers(String cluster) throws IOException {
    ...
    if (clusterDetail == null) {
      throw new IOException("Response doesn't contain `cluster`: " + response);
    }
    final Map<String, Object> module = (Map<String, Object>) clusterDetail.get("module");
    if (module == null) {
      throw new IOException("response doesn't contain `cluster.module`: " + response);
    }
    List<String> servers = (List<String>) module.get("servers");
    if (servers == null) {
      throw new IOException("response doesn't contain `cluster.module.servers`: " + response);
    }
    return servers;
  }
